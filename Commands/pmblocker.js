...
export async function before(m, { conn, isAdmin, isBotAdmin, isOwner, isROwner }) {
    ...
    if (!m.isGroup) {
      ...
      if (bot.pmblocker && !isOwner && !isROwner) {
        ...
        await m.reply(`*שלום (Shalom) @${m.sender.split`@`[0]}, messaging the bot privately is currently disabled. You have been blocked from using the bot. Contact owner if you know his contact number*`, false, { mentions: [m.sender] });
